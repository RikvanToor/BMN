...
import {
 Label, Button, ButtonGroup, Modal, Panel 
...
// Interaction imports
import {
 getSetlistSongs, addSetlistSong, updateCrew, removeSetlistSong, publishSetlistSongs 
} from '@Actions/SetlistActions.js';
...
{' '}
{this.state.songToDeleteInd >= 0 ? this.props.setlist[ind].title:''}
{' '}
wil verwijderen?
</p>),
    ...
    this.setState({ songToDeleteInd: ind, modal });
  }
  ...
  publishAll() {
    ...
    const unpublished = this.props.setlist.reduce((accum, song) => {
...
<span>
  ...
  renderSetlistSong(song, ind) {
    const label = (style, content) => (<Label bsStyle={style}>{content}</Label>);
    const ui = (
          <tr>
              <td>{song.title}</td>
              <td>{song.artist}</td>
  ...
  render() {
    ...
    const songTableHeaders = ['Titel', 'Artiest', 'Lengte', 'Bezetting', 'Gepubliceerd?', 'Acties'];
    const tableSorters = {
 0: 'title', 1: 'artist', 2: 'duration', 4: 'isPublished'
 };
    return (
          <div>
