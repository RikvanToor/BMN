...
class Navigation extends PureComponent {
  ...
  render() {
    ...
    var usersOnly = Object.keys(userLinks).map(key =>
      this.createLink(userLinks[key], key)
    );
    ...
    var committeeOnly = (
      <NavDropdown
        key="committee"
        eventKey={5}
        title="Commissie tools"
        id="basic-nav-dropdown"
      >
        {Object.keys(committeeLinks).map(key =>
          this.createMenuItem(committeeLinks[key], key)
        )}
      </NavDropdown>
    );
    ...
    if (this.props.isLoggedIn) {
      ...
      navs = navs.concat(
        <NavItem key="logOut" onClick={this.logOutUser}>
          Log uit
        </NavItem>
      );
    } else {
